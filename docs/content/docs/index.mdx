...
Pitmaster is a pure PHP library that reads and writes Git repositories natively. No `exec('git ...')`, no FFI, no extensions beyond what ships with every PHP install. The entire Git object model, pack file format, index format, and smart HTTP protocol are implemented in PHP.
...
No other extensions. No FFI. No `exec()`.
...
Pitmaster uses an oracle-driven testing model. The canonical `git` binary is the oracle: every behavior Pitmaster implements is verified by comparing its output against git's output on the same input. If `git` accepts what Pitmaster writes, and Pitmaster can read what `git` writes, it is correct. See the [Testing](/docs/testing/oracle-model) section for details.

## Next steps

- [Getting Started](/docs/getting-started) for installation and basic usage
- [API Reference](/docs/api) for the full public API
- [CLI Reference](/docs/cli) for the command-line interface
- [Operations](/docs/operations/read) for detailed operation guides
