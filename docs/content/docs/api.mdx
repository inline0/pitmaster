---
title: API Reference
description: Complete public API reference for the Pitmaster facade and Repository class.
---
...
The `Pitmaster` class is the static entry point. It provides factory methods for opening, creating, and cloning repositories, plus detection helpers.
...
The `Repository` class is the main handle for all git operations. Obtain one via `Pitmaster::open()`, `Pitmaster::init()`, or `Pitmaster::clone()`.
...
```php
public function addWorktree(string $path, string $branch, ?ObjectId $from = null): Worktree
```

Add a linked worktree. Creates the branch if it does not exist, sets up the worktree metadata, and materializes the working tree files.
