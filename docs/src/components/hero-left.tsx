...
async function getLatestVersion(): Promise<string | null> {
  try {
    const res = await fetch(
      "https://api.github.com/repos/inline0/pitmaster/releases/latest",
  ...
  return (
    <>
      ...
      <p className="text-left max-w-xl leading-normal text-fd-muted-foreground sm:text-lg sm:leading-normal text-balance mt-4">
        Read and write Git repositories without shelling out to git. No exec(),
        no FFI, no extensions. Pack files, diffs, merges, and network protocol,
        all in PHP.
      </p>
      <div className="mt-8 w-full">
        <CodeBlock
          lang="bash"
          code="composer require pitmaster/pitmaster"
          className="!my-0"
        />
        <div className="flex items-center gap-3 mt-4">
          <Button href="/docs">Get Started</Button>
          <a
            href="https://github.com/inline0/pitmaster/releases"
            target="_blank"
