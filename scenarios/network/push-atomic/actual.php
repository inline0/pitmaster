...
git('checkout topic', $sourceDir);
file_put_contents($sourceDir . '/topic.txt', "topic advance\n");
git('add topic.txt', $sourceDir);
git('commit -m topic-advance', $sourceDir);
git('push origin topic', $sourceDir);
git('checkout main', $sourceDir);

file_put_contents($cloneDir . '/main.txt', "atomic main\n");
$repo->add('main.txt');
$repo->commit("Atomic main\n");
$repo->checkout('topic');
file_put_contents($cloneDir . '/topic-local.txt', "atomic topic\n");
$repo->add('topic-local.txt');
$repo->commit("Atomic topic\n");
$repo->checkout('main');

try {
    $repo->pushAtomic('origin', ['main', 'topic']);
} catch (\Throwable) {
}
...
file_put_contents(
    ...
    . "[topic]\n"
    . git('ls-tree -r --full-tree refs/heads/topic', $remoteDir),
);
stop_server($server);
