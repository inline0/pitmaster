...
git -C "$source_dir" checkout topic >/dev/null
printf 'topic advance\n' > "$source_dir/topic.txt"
git -C "$source_dir" add topic.txt
git -C "$source_dir" commit -m topic-advance >/dev/null
git -C "$source_dir" push origin topic >/dev/null
git -C "$source_dir" checkout main >/dev/null

printf 'atomic main\n' > "$clone_dir/main.txt"
git -C "$clone_dir" add main.txt
git -C "$clone_dir" commit -m 'Atomic main' >/dev/null
git -C "$clone_dir" checkout topic >/dev/null
printf 'atomic topic\n' > "$clone_dir/topic-local.txt"
git -C "$clone_dir" add topic-local.txt
git -C "$clone_dir" commit -m 'Atomic topic' >/dev/null
git -C "$clone_dir" checkout main >/dev/null
git -C "$clone_dir" push --atomic origin main topic >/dev/null 2>&1 || true

{
  printf '[main]\n'
  git -C "$remote_dir" ls-tree -r --full-tree refs/heads/main
  printf '[topic]\n'
  git -C "$remote_dir" ls-tree -r --full-tree refs/heads/topic
} > .remote-refs.txt
