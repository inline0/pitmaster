...
try {
    ...
    Pitmaster::clone($baseUrl . '/remote.git', $cloneDir);

    $lines = [
        'head=' . trim(git('rev-parse HEAD', $cloneDir)),
        'origin.main=' . trim(git('rev-parse refs/remotes/origin/main', $cloneDir)),
        'tag.v1.0=' . trim(git('rev-parse refs/tags/v1.0', $cloneDir)),
        'readme=' . trim((string) file_get_contents($cloneDir . '/README.md')),
    ];

    file_put_contents(getcwd() . '/.clone-state', implode("\n", $lines) . "\n");
} finally {
    stop_server($server);
}
