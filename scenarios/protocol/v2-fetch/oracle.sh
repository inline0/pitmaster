...
url="$(cat .remote-url)"
trace=".git-trace-v2-fetch"
GIT_TRACE_PACKET=1 git -c protocol.version=2 clone "$url" git-clone >/dev/null 2>"$trace"
...
if ($lines === []) {
    fwrite(STDERR, "Unable to locate fetch request in trace\n");
    exit(1);
}

file_put_contents($argv[2], implode("\n", $lines) . "\n");
' "$trace" .v2-fetch-lines
